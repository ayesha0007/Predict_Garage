...
# # =========================
# user_sessions = {}
# session_envs = {}   # 🔥 NEW: persistent execution env per user
...
#         "train_test_split": __import__("sklearn.model_selection").model_selection.train_test_split,
...
#                 if now - data.get("last_active", now) > SESSION_TTL:
#                     expired.append(sid)

#             for sid in expired:
#                 user_sessions.pop(sid, None)
#                 session_envs.pop(sid, None)  # 🔥 remove env too
...
#         # 🔥 CREATE PERSISTENT ENV (IMPORTANT)
#         session_envs[session_id] = build_exec_env()

#     user_sessions[session_id]["last_active"] = time.time()

#     return jsonify({"success": True, "user_data": user_sessions[session_id]})
...
#     content = LEVEL_CONTENT.get(level, {}).get(tab, "Not found")
...
#     return jsonify({"quiz": QUIZ_DATA.get(level, [])})
...
#     correct = 0
#     for i, ans in enumerate(answers):
#         if i < len(quiz) and quiz[i]["correct"] == ans:
#             correct += 1

#     passed = correct >= 4

#     if passed and level not in user["completed_levels"]:
#         user["completed_levels"].append(level)
#         user["xp"] += correct * 5
#         user["current_level"] = min(level + 1, 8)

#     user["energy"] = max(0, user["energy"] - (5 - correct))
...
# # SAFE EXEC ENGINE
# # =========================
# def run_code(code, env, timeout=5):
#     result = {
#         "success": False,
#         "output": "",
#         "error": None,
...
#             result["output"] = output_text if output_text else "Execution complete"
#             result["image"] = img
#             result["success"] = True
...
#     if thread.is_alive():
#         return {"success": False, "error": "Timeout (5s limit)"}

#     return result


# # =========================
# # EXECUTE CODE (COLAB STYLE FIX)
# # =========================
# @app.route('/api/execute_code', methods=['POST'])
# def execute_code():
#     data = request.json
...
#             session_envs[sid] = build_exec_env()

#         return jsonify(run_code(code, session_envs[sid]))
...
#         user_sessions[sid]["dataset_features"] = features
